...
Facciamo un'esempio pratico per capire meglio il funzionamento del protocollo. Poniamo che Francesca voglia inviare un messaggio a Maurizio:
\begin{enumerate}
	\item Francesca \lstinline|alice@unipi.it| usa l'user agent per comporre un messaggio e-mail al destinatario Maurizio \lstinline|biagio@altroente.edu|;
	...
	\item Il mail server di Francesca apre una connessione TCP, comportandosi come client, con il mail server \lstinline|altroente.edu|;
	\item Il messaggio di Francesca viene trasferito verso tale mail server sulla rete TCP;
	\item Il mail server di Maurizio mette il messaggio nella mailbox di Maurizio;
	\item In un secondo momento, l'user agent di Maurizio richiede la mailbox al suo mail server, che quindi gli invia il messaggio di Francesca. 
\end{enumerate}

Qui il protcollo SMTP viene usato nella comunicazione fra l'UA di Francesca ai server di \lstinline|unipi.it|, e nella comunicazione fra questi e i server di \lstinline|altroente.edu|.
Per portare la posta dai server di \lstinline|altroente.edu| all'UA di Maurizio, invece, verrà usato un altro protocollo (come già detto IMAP o POP3).
...
Mettiamo ad esempio di leggere la comunicazione fra il server \lstinline|unipi.it| al servizio di Francesca dell'esempio precedente, quando questo inoltra la posta al mail server \lstinline|altroente.edu| di Maurizio.
